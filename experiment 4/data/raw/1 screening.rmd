...
rmts <- trimmed_df %>%
  dplyr::filter(grepl("RMTS_", task)) %>%
  dplyr::group_by(participant) %>%
  dplyr::filter(block_n == max(block_n)) %>% # because this is of a flexible number of blocks, assess only the n of trials in the last block
  dplyr::summarize(n_trials = n()) %>%
  dplyr::filter(n_trials == 16) %>%  # trim people who don't have the correct number of trials
  dplyr::select(participant)

participants_with_complete_data <- 
  dplyr::semi_join(participants_with_full_data, rmts, by = "participant")
