...
d1_summary_data <-
  data_df %>%
  group_by(condition) %>%
  dplyr::summarize(mean_d1_diff = round(mean(SCIAT_D1_change_scores), 2),
                   sd_d1_diff = round(sd(SCIAT_D1_change_scores), 2),
                   se_d1_diff = round(std.error(SCIAT_D1_change_scores), 2))

ggplot(data = d1_summary_data,
       aes(x = condition, y = mean_d1_diff)) +
  geom_pointrange(aes(ymax = mean_d1_diff + (1.96*se_d1_diff),
                      ymin = mean_d1_diff + (-1.96*se_d1_diff))) +
  ylab("SC-IAT D1") +
  xlab("Condition") +
  #coord_cartesian(ylim = c(-.75,.75)) +
  scale_colour_grey() +
