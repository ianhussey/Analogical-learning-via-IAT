...
data_df <- 
  data_df %>%
  dplyr::rename(likert_Pre_item = likertPre.thisTrialN,
  ...
  dplyr::mutate(accuracy = abs(accuracy - 1), # recitfies the direction of accuracy so that 0 = error and 1 = correct.
...
Likert_chinese_data <-
  ...
  dplyr::summarize(ratings_pre = round(mean(rating[likertTimepoint == "likert_Pre_item"]), 2),
...
Likert_flowerInsects_data <-
  ...
  dplyr::select(participant,
                ...
                likert_Valence_item) %>%
  group_by(participant) %>%
  dplyr::summarize(flowers_ratings = round(mean(rating[subscale == 2]), 2),
...
IAT_D1_df <-
  data_df %>%
  dplyr::filter(IAT == 0) %>% # select only rows that corrispond to the IAT data
  group_by(participant) %>%
  dplyr::filter(rt <= 10000) %>%
  dplyr::summarize(IAT_mean_RT = round(mean(rt[IAT_blocks == 3 | IAT_blocks == 4 | IAT_blocks == 6 | IAT_blocks == 7])*1000, 0), 
  ...
  dplyr::mutate(differenceA = RT_block6_m - RT_block3_m,
...
SCIAT1_D1_df <-
  ...
  group_by(participant) %>%
  filter(rt <= 10000) %>%
  dplyr::summarize(SCIAT1_mean_RT = round(mean(rt[SCIAT1_blocks == 2 | SCIAT1_blocks == 3])*1000, 0), 
  ...
  dplyr::mutate(difference = RT_block3_m - RT_block2_m,
...
SCIAT2_D1_df <-
  ...
  group_by(participant) %>%
  filter(rt <= 10000) %>%
  dplyr::summarize(SCIAT2_mean_RT = round(mean(rt[SCIAT2_blocks == 2 | SCIAT2_blocks == 3])*1000, 0), 
...
IAT_condition_df <-
  ...
  group_by(participant) %>%
  distinct(condition) %>%
  dplyr::rename(IAT_condition = condition) %>%
  dplyr::select(participant, 
                IAT_condition) %>%
  dplyr::mutate(valence_of_IAT_contrast_category = ifelse(IAT_condition == 1 | IAT_condition == 2,
                                                          "flowers", 
                                                          ifelse(IAT_condition == 3 | IAT_condition == 4,
                                                                 "insects")),
                IAT_block_order = ifelse(IAT_condition == 1 | IAT_condition == 4, 
                                     "consistent_first", 
                                     ifelse(
...
IAT_accuracy_data <- 
  ...
  filter(IAT == 0) %>% 
  group_by(participant) %>%
  dplyr::summarize(IAT_accuracy = round(sum(accuracy[IAT_blocks == 3 |
                                                       IAT_blocks == 4 | 
                                                       IAT_blocks == 6 | 
...
SCIAT1_accuracy_data <- 
  ...
  filter(SCIAT1 == 0) %>% 
  group_by(participant) %>%
  dplyr::summarize(SCIAT1_accuracy = round(sum(accuracy[SCIAT1_blocks == 2 | 
...
SCIAT2_accuracy_data <- 
  ...
  filter(SCIAT2 == 0) %>% 
  group_by(participant) %>%
  dplyr::summarize(SCIAT2_accuracy = round(sum(accuracy[SCIAT2_blocks == 2 |
...
output_df <- 
  all_tasks_df %>%
  dplyr::mutate(IAT_D1 = ifelse(IAT_condition == 1 | IAT_condition == 3, IAT_D1 *-1, IAT_D1),
                SCIAT1_D1 = ifelse(SCIAT_block_order == 2, SCIAT1_D1 *-1, SCIAT1_D1),
                SCIAT2_D1 = ifelse(SCIAT_block_order == 2, SCIAT2_D1 *-1, SCIAT2_D1))

## Difference scores
output_df <-
  output_df %>%
  dplyr::mutate(ratings_change_scores = ratings_post - ratings_pre,
...
output_df <- 
  ...
  dplyr::select(participant,
                ...
                SCIAT1_accuracy,
                SCIAT2_accuracy) %>%
  dplyr::rename(condition = valence_of_IAT_contrast_category)
